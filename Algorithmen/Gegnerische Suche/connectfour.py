...
def generate_segments(num_columns: int, num_rows: int, segment_length: int) -> List[List[Tuple[int, int]]]:
    ...
    # Die vertikalen Segmente erzeugen
    ...
    # Die horizontalen Segmente erzeugen
    ...
    # Die diagonalen Segmente von links unten nach rechts oben erzeugen
    for c in range(num_columns - segment_length + 1):
        for r in range(num_rows - segment_length + 1):
            segment = []
            for t in range(segment_length):
    ...
    # Die diagonalen Segmente von links oben nach rechts unten erzeugen
    for c in range(num_columns - segment_length + 1):
...
class C4Board(Board):
    ...
    class Column:
        ...
        def push(self, item: C4Piece) -> None:
            if self.full:
                raise OverflowError("Versuch, Stein in volle Spalte zu werfen")
    ...
    # Gibt die Anzahl der schwarzen und roten Steine in einem Segment zurück
    ...
    def _evaluate_segment(self, segment: List[Tuple[int, int]], player: Piece) -> float:
        ...
        if red_count > 0 and black_count > 0:
            return 0  # Gemischte Segmente sind neutral
