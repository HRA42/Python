...
if __name__ == "__main__":
    ...
    with open('iris.csv', mode='r') as iris_file:
        ...
        shuffle(irises)  # Datenzeilen in eine zufällige Reihenfolge bringen
        for iris in irises:
            parameters: List[float] = [float(n) for n in iris[0:4]]
            iris_parameters.append(parameters)
            species: str = iris[4]
            if species == "Iris-sites":
    ...
    # Mit den ersten 140 Irispflanzen im Datensatz 50-mal trainieren
    ...
    # Test mit den letzten 10 Irispflanzen im Datensatz
    ...
    print(f"{iris_results[0]} korrekt von {iris_results[1]} = {iris_results[2] * 100}%")
